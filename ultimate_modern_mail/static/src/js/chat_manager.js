...
//----------------------------------------------------------------------------------
    ...
    function notify_incoming_message(group, msg, options) {
        ...
        if (msg.author_id[1] && group == '') {
            title = _.escape(msg.author_id[1]);
        } else {
            title = group + ' / ' + _.escape(msg.author_id[1]);
        }
...
// options: channel_id, silent
                ...
                if (channel) {
                    ...
                    if (channel.type !== 'static' && !msg.is_author && !msg.is_system_notification) {
                        ...
                        if (options.show_notification) {
                            ...
                            var group = channel.is_chat ? '' : channel.name;

                            notify_incoming_message(group, msg, query);
                        }
                    }
                }
            });
    ...
    function edit_to_cache(message, domain) {
        _.each(message.channel_ids, function (channel_id) {
            ...
            if (channel) {
                ...
                if (channel_cache.messages[index].id === message.id) {
                    channel_cache.messages[index] = message;
                }
            }
        });
    }

    function remove_to_cache(message, domain) {
        _.each(message.channel_ids, function (channel_id) {
            var channel = chat_manager.get_channel(channel_id);
            if (channel) {
                var channel_cache = get_channel_cache(channel, domain);
    ...
    function make_message(data) {
        var msg = {
            ...
            body: emojione.unicodeToImage(data.body || ""),
    ...
    function on_channel_notification(message) {
        ...
        def.then(function () {
            ...
            if (message['action'] === 'edit') {
                edit_message(message, {show_notification: true});
            } else if (message['action'] === 'del') {
                deleteMessage(message, {show_notification: true});
            } else {
    ...
    var chat_manager = {
        ...
        post_message: function (data, options) {
            ...
            if ('channel_id' in options) {
                ...
                var data = _.extend(msg, {
                    message_type: 'comment',
                    content_subtype: 'html',
                    subtype: 'mail.mt_comment',
                    command: data.command,
                })
                return ChannelModel.call(data.command ? 'execute_command' : 'message_post', [options.channel_id], data);
            }
        ...
        edit_message: function (data, message) {
            var body = utils.parse_and_transform(_.str.trim(data.content), utils.add_link);

            var msg = {
                body: body,
            };
            return MessageModel.call('edit_message', [[message.id], body]);
        },
        ...
        delete_message: function (message_id) {
            return MessageModel.call('delete_message', [[message_id]]);
        },
        unstar_all: function () {
