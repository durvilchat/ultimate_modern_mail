    ...
    var ChatAction = Widget.extend(ControlPanelMixin, {
        ...
        events: {
            ...
            "click .o_mail_partner_unpin": function (event) {
                ...
                var userselection = confirm(_t('Do you want to leave this conversation channel?'));
                if (userselection) {
            ...
            "click .o_mail_request_permission": function (event) {
                ...
                if (def) {
                    def.then(function (value) {
                        if (value === 'denied') {
                            utils.send_notification(_t('Permission denied'), _t('Your app will not have the permission to send native notifications on this device.'));
                        } else {
                            utils.send_notification(_t('Permission granted'), _t('Your app has now the permission to send you native notifications on this device.'));
                        }
                    });
                }
            },
        ...
        start: function () {
            ...
            this.thread.on('delete_message', this, function (message_id) {
                chat_manager.delete_message(message_id);
            });
            ...
            this.thread.on('select_message_edit', this, this._selectMpMessage);
            ...
            this.basic_composer.on('cancel_edit_message', this, this._onCancelEditMessage);
         ...
         */
        _selectMpMessage: function (message_id) {
            var message = chat_manager.get_message(message_id);
            // first create an element and add the string as its HTML
            var container = $('<div>').html(message.body);
            // then use .replaceWith(function()) to modify the HTML structure
            container.find('img').replaceWith(function () {
                return this.alt;
            });
            this.selected_messageMP = message;
            this.basic_composer.cancelEditBututom('show');
            this.basic_composer.set_input(container.text());
        },
        ...
        _onCancelEditMessage: function () {
            this.basic_composer.cancelEditBututom('hide');
            this.selected_messageMP = null;
        },
        ...
        on_post_message: function (message) {
            ...
            if (this.selected_messageMP) {
                chat_manager
                    .edit_message(message, this.selected_messageMP).then(function () {
